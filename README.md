...
.cursor/         — Cursor `stop` hook that auto-runs `chunk validate` (same loop, in Cursor)
...
A stop hook runs `chunk validate` automatically every time the agent finishes a turn — that's the inner-loop magic. This repo wires it for both agents:

- **Claude Code:** `.claude/settings.json` `Stop` hook (generated by `chunk init`).
- **Cursor:** `.cursor/hooks.json` `stop` hook → `.cursor/hooks/chunk-validate.sh`, which runs `chunk validate` and, on failure, feeds the gate output back to the agent as a `followup_message` so it fixes the issue and validation re-runs (bounded by `loop_limit`).
